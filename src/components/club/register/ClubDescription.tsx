...
const ClubDescription = ({ className }: ClubDescriptionProps) => {
  const {
    clubName,
    setClubName,
    description,
  ...
  return (
    <div className={`desc-container ${className}`}>
      <label className="up-wrapper">
        <Label labelTitle="모임 소개" isRequired htmlFor="club-title" />
        <input
          id="club-title"
          type="text"
        ...
        />
        {!isTitleValid && <span className="err-message">{titleError}</span>}
      </label>
      <div className="desc-wrapper">
        ...
        {!isDescValid && <span className="err-message">{descError}</span>}
      </div>
    </div>
  );
};
